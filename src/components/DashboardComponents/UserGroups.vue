<template>
  <v-card class="flex-grow-1 vh-dashboard-groups card-rounded" flat>
    ...
    <v-tabs grow>
      ...
      <v-tab>
        <v-icon start>mdi-timer-plus</v-icon>
        {{ $t('memberships') }}
      </v-tab>
      <v-tab>
        ...
        <v-card class="card-rounded" flat v-if="dataFetched >= 2">
          ...
          <v-card-text>
            <v-btn v-if="$checkProp('user_edit')" color="success"
                   @click="$refs.addMembershipDialog.show()" variant="outlined">
        ...
        <v-card>
          <v-data-table
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn variant="outlined" color="primary" size="small" v-if="$checkProp('user_edit')"
